...
async function initializeGoogleDrive() {
    ...
    if (process.env.GOOGLE_DRIVE_KNOWLEDGE_BASE_FOLDER_ID) {
        ...
        console.warn('GOOGLE_DRIVE_KNOWLEDGE_BASE_FOLDER_ID not set. Attempting to find "sueAssistantKnowledgeBase" folder by name. For production, hardcode the ID or set the env var.');
        try {
            const res = await driveClient.files.list({
                q: "name='sueAssistantKnowledgeBase' and mimeType='application/vnd.google-apps.folder'",
            ...
            if (folder && folder.id) {
                ...
                console.log(`Found "sueAssistantKnowledgeBase" folder with ID: ${knowledgeBaseFolderId}`);
            } else {
                throw new Error('Google Drive folder "sueAssistantKnowledgeBase" not found. Please create it and/or set GOOGLE_DRIVE_KNOWLEDGE_BASE_FOLDER_ID.');
            }
        } catch (error) {
